...
> 证据状态：项目实测（2026-09-23 冷启动、请求抓取、用量汇总、窗口关闭后服务存活、二次启动复用均实测通过；2026-09-24 历史删除、Token 估算口径与大会话删除性能修复实测通过）
...
> 更新时间：2026-09-24（补充：左侧历史单条/批量/整会话删除；估算规则改为整包估算并重新标定 CJK 权重；修复大会话删除后刷新卡死）
...
12. **大会话删除曾表现为“失效”，根因不是删除接口，而是删除后的全量刷新。** 2026-09-24 实测 `2026-09-24T11-08-38-398` 会话已有 1818 个清单、约 627 MB：旧实现 `/api/requests` 约 66 秒、`/api/session-stats` 约 101 秒，删除文件本身成功，但前端删完立刻重拉整会话，界面长时间无响应，用户看到的是“没删掉”。修复分三层：① `store.js` 增加按文件 `mtime+size` 的摘要缓存与异步分片读取，列表/统计不再每次解包全部消息 blob；② 删除接口改用 `rmEntriesMultiFast` 先返回、后台 `gcBlobsAsync` 回收孤儿 blob；③ 前端删除后按返回 id 本地移除并轻量更新统计，不再触发同步全量重载。实测：列表 283 ms、统计 283 ms；大会话统计进行中并发删除 119 ms 返回且文件立即消失。旧 v2 清单缺少 `metrics` 时会在后台串行回填预估 Token 与工具调用数。
...
- **大会话点删除后界面长时间没反应**：这是旧版删除后全量刷新导致的阻塞，不是删除失败。2026-09-24 修复后列表/统计走摘要缓存与异步分片，删除响应先落盘再后台回收 blob；若仍卡住，确认 `9911`/`9912` 已重启加载新后端，并强刷看板使 `app.js?v=20260924-delete-fast` 生效。
...
- [2026-09-24 CodexTokenGlass 大会话删除失效修复](../来源记录/2026-09-24_CodexTokenGlass大会话删除失效修复.md)
