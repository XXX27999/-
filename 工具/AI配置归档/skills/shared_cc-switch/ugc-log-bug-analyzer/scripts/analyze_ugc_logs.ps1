...
function Resolve-ProjectLogRoot {
    param([string]$ExplicitRoot, [string]$WorkingDirectory)

    if (-not [string]::IsNullOrWhiteSpace($ExplicitRoot)) {
        return [System.IO.Path]::GetFullPath($ExplicitRoot)
    }

    $currentDirectory = [System.IO.DirectoryInfo]::new([System.IO.Path]::GetFullPath($WorkingDirectory))
    while ($null -ne $currentDirectory.Parent) {
        if ($currentDirectory.Parent.Name -ieq 'UGCProjects') {
            $projectName = $currentDirectory.Name
            $shadowTrackerExtraRoot = $currentDirectory.Parent.Parent
            if ($null -eq $shadowTrackerExtraRoot) {
                throw "Cannot resolve ShadowTrackerExtra root from working directory: $WorkingDirectory"
            }
            return Join-Path $shadowTrackerExtraRoot.FullName (Join-Path 'Saved\Logs' $projectName)
        }
        $currentDirectory = $currentDirectory.Parent
    }

    throw "Cannot infer project log root. Run from UGCProjects/<ProjectName> or pass -Root explicitly. WorkingDirectory=$WorkingDirectory"
}

$Root = Resolve-ProjectLogRoot -ExplicitRoot $Root -WorkingDirectory (Get-Location).Path
...
function Normalize-ScriptFunctionTarget {
    param([string]$Target)
    if ([string]::IsNullOrWhiteSpace($Target)) { return $null }
    $normalized = $Target.Trim()
    $normalized = $normalized.Trim('[', ']')
    $leftBracket = [string][char]0x3010
    $rightBracket = [string][char]0x3011
    if ($normalized.StartsWith($leftBracket) -and $normalized.EndsWith($rightBracket)) {
        $normalized = $normalized.Substring(1, $normalized.Length - 2)
    }
    $parts = $normalized.Split('+', 2)
    if ($parts.Count -ne 2) { return $normalized }
...
function Get-ScriptFunctionMatch {
    param([string]$Line, [string[]]$Targets)
    foreach ($target in $Targets) {
        if ([string]::IsNullOrWhiteSpace($target)) { continue }
        $needle = ([string][char]0x3010) + $target + ([string][char]0x3011)
        if ($Line.IndexOf($needle, [System.StringComparison]::OrdinalIgnoreCase) -ge 0) { return $target }
        if ($Line.IndexOf($target, [System.StringComparison]::OrdinalIgnoreCase) -ge 0 -and $Line.IndexOf('[UGC]', [System.StringComparison]::Ordinal) -ge 0) { return $target }
    }
    return $null
}
...
function Analyze-Logs {
    param([string[]]$Files, [int]$WindowRadius, [int64]$MaxBytes, [bool]$ReadFullFile, [string[]]$FocusKeywords, [string[]]$ScriptFunctionTargets)
    ...
    $targetList = @($ScriptFunctionTargets | ForEach-Object { Normalize-ScriptFunctionTarget -Target $_ } | Where-Object { -not [string]::IsNullOrWhiteSpace($_) } | Select-Object -Unique)
    $targetedMode = $targetList.Count -gt 0
    $focusSignals = [ordered]@{ count = 0; samples = New-Object 'System.Collections.Generic.List[object]' }
    $targetSignals = [ordered]@{ count = 0; samples = New-Object 'System.Collections.Generic.List[object]' }
    $targetCounts = @{}
    ...
    foreach ($file in $Files) {
        ...
        $fileStats[$file] = [ordered]@{ lines = 0; bytes = $scanPlan.bytes; scanned_bytes = $scanPlan.scanned_bytes; scan_start_offset = $scanPlan.scan_start_offset; truncated = $scanPlan.truncated; hits = 0; focus_hits = 0; target_hits = 0 }
        $lineNumber = 0
        ...
        Read-LogLines -File $file -StartOffset $scanPlan.scan_start_offset | ForEach-Object {
            ...
            $lineMatched = $false
            $targetMatch = Get-ScriptFunctionMatch -Line $line -Targets $targetList
            if ($null -ne $targetMatch) {
                $lineMatched = $true
                if ($null -eq $timestamp) { $timestamp = Get-TimestampFromLine -Line $line }
                if (-not $targetCounts.ContainsKey($targetMatch)) { $targetCounts[$targetMatch] = [ordered]@{ count = 0; first = $timestamp; last = $timestamp; files = @{} } }
                $targetCounts[$targetMatch].count++
                $targetCounts[$targetMatch].last = $timestamp
                if (-not $targetCounts[$targetMatch].files.ContainsKey($file)) { $targetCounts[$targetMatch].files[$file] = 0 }
                $targetCounts[$targetMatch].files[$file]++
                $targetSignals.count++
                $fileStats[$file].target_hits++
                if ($targetSignals.samples.Count -lt 6) {
                    $excerpt = if ($WindowRadius -gt 0) { Get-RecentContextExcerpt -RecentLines $recentLines -Line $line } else { $line }
                    $targetSignals.samples.Add((New-Sample -Key ($file + '|target|' + $targetMatch + '|' + $lineNumber) -File $file -ScanLine $lineNumber -ScanPlan $scanPlan -Timestamp $timestamp -Excerpt $excerpt)) | Out-Null
                }
            }
    ...
    if ($targetedMode) {
        $targetMatches = @($targetCounts.GetEnumerator() | Sort-Object { $_.Value.count } -Descending | ForEach-Object {
            [ordered]@{ target = $_.Key; count = $_.Value.count; first = $_.Value.first; last = $_.Value.last; files = @($_.Value.files.GetEnumerator() | ForEach-Object { [ordered]@{ file = $_.Key; count = $_.Value } }) }
        })
        $targetEvidence = @($targetSignals.samples.ToArray() | ForEach-Object {
            [ordered]@{ file = $_.file; line = $_.line; scan_line = $_.scan_line; scan_start_offset = $_.scan_start_offset; timestamp = $_.timestamp; excerpt = $_.excerpt }
        })
        if ($targetSignals.count -gt 0) {
            Add-Issue -Bucket $issues -Severity 'medium' -Confidence 0.86 -Title 'Targeted script/function log evidence' -Summary "Matched $($targetSignals.count) log lines for the requested script/function target(s); only targeted evidence was returned." -Evidence $targetEvidence -RootCause 'The target log path executed and emitted evidence. Use the timestamps, side, arguments, and state transitions in these lines to locate the failing branch.' -Suggestions @('Compare entry, key data, branch result, and exit logs for the same target.', 'If the target has no exit log, inspect the last matched line and add a protected exit/error log.') -MatchedPatterns @('script-function-target')
        }
        else {
            Add-Issue -Bucket $issues -Severity 'low' -Confidence 0.58 -Title 'No targeted script/function log found' -Summary "No log line matched the requested target(s): $($targetList -join ', ')." -Evidence @() -RootCause 'The target may not have executed in the selected DebugID, its tag may use a different script/function spelling, or the relevant log is outside the selected session.' -Suggestions @('Confirm the exact script+function tag spelling.', 'Check the selected DebugID and rerun with -FullScan when the event may be earlier than the tail window.') -MatchedPatterns @('script-function-target-missing')
        }
        return [ordered]@{
            Issues = $issues
            Signals = [ordered]@{
                targeted_mode = $true
                target_queries = $targetList
                target_hit_count = $targetSignals.count
                target_matches = $targetMatches
                target_samples = $targetEvidence
                scan_policy = [ordered]@{ full_scan = $ReadFullFile; max_bytes_per_file = $MaxBytes; truncated_files = @($truncatedFiles.ToArray()) }
                scanned_files = @($fileStats.Keys | ForEach-Object { [ordered]@{ file = $_; lines = $fileStats[$_].lines; bytes = $fileStats[$_].bytes; scanned_bytes = $fileStats[$_].scanned_bytes; scan_start_offset = $fileStats[$_].scan_start_offset; truncated = $fileStats[$_].truncated; target_hits = $fileStats[$_].target_hits } })
            }
        }
    }
...
try {
    ...
    $analysis = Analyze-Logs -Files $selectedFiles -WindowRadius $ContextLines -MaxBytes $MaxBytesPerFile -ReadFullFile ([bool]$FullScan) -FocusKeywords $FocusKeyword -ScriptFunctionTargets $ScriptFunction
    $status = 'ok'
