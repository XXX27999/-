...
- 技能仓库固定安装在 `D:\oasis-skill-plus`，脚本从该路径读取。
- 官方文档从 `D:\oasis-skill-plus/docs/api` 和 `D:\oasis-skill-plus/docs/wiki` 读取。
- 适用 `--project-root` 时，优先使用 `D:\oasis-skill-plus` 固定路径，兜底回退到 `<project-root>/oasis-skill-plus` 子模块路径。
- 使用附带脚本进行稳定查询：

```bash
node D:\oasis-skill-plus\skills\oasis-official-docs\scripts\query-oasis-docs.mjs \
...
- 在 `D:\oasis-skill-plus` 仓库根目录执行刷新命令：
