...
和平精英通过 [GameplayTag](../20102_Gameplay标签.md) 的方式表示角色状态，每一种状态都是独立的标签，相较于传统的枚举类型，标签化的优势在于方便动态拓展且能实现树状的查找和父节点匹配，尤其在技能和Buff的复杂状态逻辑判定中更加灵活高效。
...
![image.png](../../../_assets/images/27da90a4_DpmWKimage.png)
...
[GameplayTag](../20102_Gameplay标签.md) 已经预设了和平精英人物角色所有的状态标签和部分功能系统扩展的特殊状态标签，开发者可以直接应用于角色执行特定行为前的状态判定，各类状态标签说明可参见 [角色状态标签](../属性与伤害/20205_角色属性与状态.md#角色状态标签)。
...
如果开发者构建了新的 [状态Tag](../20102_Gameplay标签.md#添加标签)，并且希望为该状态赋予互斥关系，或者在预设的和平角色状态互斥规则基础上新增互斥关系，需要在业务逻辑中实现状态被打断的逻辑处理、是否被禁用的前置判定以及主动打断/禁用其他状态。
...
![image.png](../../../_assets/images/311a8ead_i6sOjimage.png)

编辑器预设了一套通用的状态互斥关系规则，双击打开互斥规则表 ```DA_UGCTagMappingTemplate``` 即可查看具体的配置。

![image.png](../../../_assets/images/aa209cc4_tektWimage.png)
...
![image.png](../../../_assets/images/01cfec14_5XqL8image.png)
...
1. 于内容浏览器右键 ``各种各样 -> 数据资源``，基于 ``DynamicStateTagRelationshipMapping`` 类创建新的数据资产表。
	...
	![image.png](../../../_assets/images/29aeb95a_q9DWSimage.png)

2. 在资产表中从0开始配置新的互斥规则。

	![image.png](../../../_assets/images/a5421469_N6Z1uimage.png)

3. 将 ``PersistClientStateComponent`` 组件的 ```Tag Relationship Mapping``` 属性设置为新的数据资产表。

	![image.png](../../../_assets/images/b66e76e4_7sIMMimage.png)
...
![image.png](../../../_assets/images/017f125b_NQDVvimage.png)

以 ``践踏`` 技能蓝图为例，查看蓄力（Charge）阶段配置，此阶段技能轨道上添加了一个 [技能Task-状态变化](./技能编辑器2.0/20094_技能Task查询手册.md#技能Task-状态变化) 用于设置需要打断和禁用的状态，效果为玩家在施放技能的蓄力阶段无法移动。

![image.png](../../../_assets/images/f2da597b_xEKh2image.png)

更多关于技能中的 [技能Task-状态变化](./技能编辑器2.0/20094_技能Task查询手册.md#技能Task-状态变化) 和Buff中的 [状态互斥](./Buff编辑器2.0/20087_Buff编辑器.md#状态互斥) 设置说明可参阅相应的文档。
