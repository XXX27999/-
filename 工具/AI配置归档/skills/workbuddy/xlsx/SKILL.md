...
display_name_en: "Excel Spreadsheets"
description: >-
  Full lifecycle for Excel files: create workbooks with formulas and formatting, build charts and pivot tables, read and clean data, batch-edit cells, and apply conditional formatting.
  全面处理 .xlsx/.xlsm 表格：创建带公式与格式的表格、生成图表与数据透视、读取与清洗数据、批量修改单元格、条件格式与数据验证，交付 Excel 类成果时必用。
  触发词 / Trigger words：Excel 表格、xlsx、做个表、spreadsheet、pivot table、excel chart、公式计算
description_zh: "创建、编辑、分析 Excel 表格：公式、图表、透视表与格式设置。"
description_en: "Creates, edits, and analyzes Excel files: formulas, charts, pivot tables, formatting."
category: document
version: "1.0.0"
author: "zxh"
disable-model-invocation: false
user-invocable: true
title: "Excel 表格处理 / Excel Spreadsheets"
runtime: generic
tags: ["Excel", "xlsx", "表格", "公式", "charts", "pivot table", "spreadsheet"]
dependencies: []
agent_created: true
---
...
> **语言 / Language**：中文输入请先阅读 `references/zh-CN.md`，并全程使用中文输出；其他语言直接使用下方英文指南。
> For Chinese input, read `references/zh-CN.md` first and respond in Chinese throughout; otherwise follow the English guide below.
...
`openpyxl`, `pandas`, `markitdown` (pip, preinstalled — install only if an import fails or the command is missing) · LibreOffice (`soffice`, auto-configured for sandboxed environments via `scripts/soffice.py`)
