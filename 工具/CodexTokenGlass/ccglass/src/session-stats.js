...
import { estimateResponseTokens } from "./tokens.js";
...
export function aggregateSessionStats(records, options = {}) {
  ...
  for (const rec of scoped) {
    ...
    totalUsd += c.usd ?? 0;
    // Estimated whole-request tokens (request body + response content), summed
    // so the header can be compared against the provider's real bill.
    estTotal += A.estimateTokens(body) + estimateResponseTokens(parsed);
    completed++;
  }
