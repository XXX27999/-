...
![image.png](../../../_assets/images/7de6796d_j4j2fimage.png)
...
![image.png](../../../_assets/images/d94fbb52_EpXkkimage.png)

在【表格类型区】选择“功能表格”，然后在【表格浏览区】的【Template】单击选择“怪物组表”，点击“选择模板创建”后，成功创建 ``UGCMobGroup`` 怪物组表。

![怪物组表.drawio.png](../../../_assets/images/de08af7b_fbOFe怪物组表.drawio.png)

双击打开 ``UGCMobGroup`` 怪物组表，可配置多组生成规则，每组包含怪物的种类、阵营、生成权重和初始属性值等属性。

![image.14.png](../../../_assets/images/22c1cd0f_gl5Joimage.14.png)

|属性名|属性说明|
|-|-|
|怪物组ID|每组生成规则的唯一ID，被刷新点引用|
|怪物阵营|怪物所属的 [阵营](../角色系统/20095_队伍与阵营.md#阵营系统)|
|怪物类型|基于实体编辑器创建的 [怪物蓝图](./20144_怪物快速配置指南.md#创建怪物蓝图)|
...
![image.png](../../../_assets/images/112eaa68_5skdJimage.png)
![image.png](../../../_assets/images/442b688a_2xPBYimage.png)

### 配置刷新点

怪物生成的位置取决于刷新点的坐标位置，因此需要把刷新点蓝图放置在关卡中期望刷出怪物的目标位置，此时将生成实例化对象。

![image.png](../../../_assets/images/5b320569_5cRN6image.png)

选中关卡中的刷新点实例对象，在实例对象【细节】面板编辑刷新点属性。

![image.png](../../../_assets/images/376860f8_vVgN9image.png)

|属性名|属性说明|
|-|-|
|刷出的怪物阵营|默认为-1表示使用 [怪物蓝图中配置的阵营](../角色系统/20095_队伍与阵营.md#设置怪物阵营) ，若大于-1则会将此数据覆盖到刷出的怪物上|
...
|使用NavMesh找随机点|是否优先在被 [导航网格](./20266_导航网格.md) 覆盖的范围内刷怪|
...
![企业微信截图_17503144356318.png](../../../_assets/images/de4727ab_kRT9f企业微信截图_17503144356318.png)

怪物蓝图配置表：

![企业微信截图_17503144975179.png](../../../_assets/images/6d3399eb_0KUql企业微信截图_17503144975179.png)

怪物属性配置表：

![image.png](../../../_assets/images/f4a8a772_GA8vaimage.png)
...
![image.png](../../../_assets/images/6e69dc8a_K81tCimage.png)
...
![image.png](../../../_assets/images/ab3bc71c_WfUcRimage.png)

## 刷怪管理器

使用刷怪管理器生成怪物时，需要创建刷怪管理器蓝图，编辑器内容浏览器右键 ``蓝图类``，继承自 ``BP_UGCMobSpawnerManager`` 创建子蓝图类，同样需要将刷怪管理器蓝图放置到关卡中实例化才能生效。

![1111.png](../../../_assets/images/c9bde97c_oSotF1111.png)

在关卡中选中刷怪管理器实例对象，在实例对象【细节】面板编辑配置属性。

![image.png](../../../_assets/images/dd4374f0_8WiV7image.png)
...
|事件名|启动方式为“事件触发”时，绑定的指定 [GMP事件](../../../通用功能/20163_通用消息系统.md)|
|每帧刷怪上限|每一帧执行怪物生成的数量限制，防止游戏卡顿|
|存活怪物检查间隔|检查怪物存活状态的时间间隔，``波次开始条件``设置为“All Mob Die”时会实时检测刷出的怪物是否已死亡，理论上间隔越大性能消耗越小，但时间误差也相对越大|
|刷怪点|该波次执行的刷新点，可手动选择蓝图类，也可以通过吸管工具吸取关卡中的刷新点实例对象<br>![QQ202571-12644-HD-ezgif.com-video-to-gif-converter.gif](../../../_assets/images/43735862_7MWv7QQ202571-12644-HD-ezgif.com-video-to-gif-converter.gif)|
