...
- 新建/复制后先 `save_package()` 再 `load_object`；找不到时用 `resolve_asset`，禁止凭命名拼路径。
  注意 `ue.save_package` 不存在，它是对象方法（`dt.save_package()` / `st.save_package()`），也可用 `ue.objed_save_asset(资产名, 类型)`。
- DataTable 建表前必须 `objed_open_editor('DataTable')`，否则报 `Cannot get asset handler`。
  `自定义表格` 唯一模板是 `空表格`，建出来的行结构体只有单列 `MemberVar_0`，须补成项目规定的四列字符串；
  删列 `struct_remove_variable` 入参必须是 `FGuid`（传索引或名称报 `object is not a FGuid`），加列可直接 `clone()` 现有四列结构体的变量描述体。
- `data_table_add_row` 成功时返回 `None`，**不能当失败判据**，必须回读 `data_table_as_dict()` 确认。
