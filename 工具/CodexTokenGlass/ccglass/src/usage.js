...
function addInto(bucket, cost, est = {}) {
  bucket.requests += 1;
  ...
  bucket.estInput += est.input || 0;
  bucket.estOutput += est.output || 0;
  bucket.usd += cost.usd || 0;
}

function deriveTotals(b) {
  const cacheHitRate = b.totalInput ? b.cacheRead / b.totalInput : 0;
  const actualTotal = b.input + b.output + b.cacheRead + b.cacheWrite;
  const estTotal = b.estInput + b.estOutput;
  return {
    ...b,
    cacheHitRate,
    // Estimated whole-request tokens (input + output) and how far they sit from
    // the provider's own bill. `actualTotal` is gross input (uncached + cached +
    // cache write) plus output, which is what a full round-trip costs.
    estTotal,
    actualTotal,
    estDeviation: actualTotal ? (estTotal - actualTotal) / actualTotal : 0,
  };
}
...
// Estimated token split for one record: the whole request body (which is what
// the provider bills as input) plus the reassembled response content. Kept next
// to the exact usage so the summary can show both and their deviation.
function estimateFor(rec, cost) {
  const adapter = getAdapter(detectFormat(rec));
  const resp = rec.response?.raw ? adapter.reassemble(rec.response.raw) : rec.response || {};
  return {
    input: estimateRequestTokens(rec.request?.body || {}),
...
export function summarizeUsage(roots, { names = false } = {}) {
  ...
  for (const session of listSessionsMulti(roots)) {
    ...
    for (const rec of recs) {
      ...
      const est = estimateFor(rec, cost);
      addInto(totals, cost, est);
      addInto(sessionBucket, cost, est);

      if (!byModelMap.has(model)) byModelMap.set(model, blankBucket());
      addInto(byModelMap.get(model), cost, est);
    }
