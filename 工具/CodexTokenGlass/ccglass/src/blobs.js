...
// Async variant used after dashboard deletions. The sync version above is kept
// for CLI calls; this one yields between files so a large GC does not freeze
// the collector's event loop while Codex is streaming.
export async function gcBlobsAsync(root, listSessions, sessionDir) {
  const used = new Set();
  for (const s of listSessions(root)) {
    const dir = sessionDir(root, s);
    let files;
  ...
  let shards;
  try {
    shards = await fs.promises.readdir(blobsDir);
  } catch {
    return;
  }
  for (const shard of shards) {
    ...
    let blobFiles;
    try {
      blobFiles = await fs.promises.readdir(shardDir);
    } catch {
      continue;
    }
    for (const bf of blobFiles) {
      ...
      if (!used.has(ref)) {
        try {
          await fs.promises.rm(path.join(shardDir, bf), { force: true });
        } catch {
          /* best effort */
        }
      }
